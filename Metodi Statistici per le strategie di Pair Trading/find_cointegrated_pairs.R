                                    ...
                                    plot=FALSE)
{
  ...
  if(plot){
    ...
    pl <- plot(train_real[,c(f_ticker,s_ticker)],
               main=paste(f_ticker,'-',s_ticker,'Formation Period'),
               legend.loc='topleft',grid.col=NA,col=c('black','red'))
    plot(pl)
    
    pl_2 <- plot(diff(train_real[,c(f_ticker)]),
               main=bquote(Delta~'DIA.MI'),legend.loc='topleft',grid.col=NA)
...
out <- find_cointegrated_pairs("ftsemib_2015-2020.csv",cut_2020=F,
                                  method='log',
                                  split_date = '2019-01-01',plot = T)
